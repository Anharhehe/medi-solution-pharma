...
import { nutraceuticals } from '../../../data/nutraceuticals';
import { 
...
export default function NutraceuticalDetailPage() {
  const params = useParams();
  const id = parseInt(params.id as string);
  const nutraceutical = nutraceuticals.find(a => a.id === id);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-red-50 to-pink-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-6">The nutraceutical you're looking for doesn't exist.</p>
          <Link href="/nutraceuticals" className="bg-blue-600 text-white px-5 py-2.5 rounded-lg hover:bg-blue-700 transition-colors">
              ...
              <div className="bg-white rounded-xl shadow-xl overflow-hidden">
                ...
                <div className="relative h-121 overflow-hidden">
                  ...
                  <div className="relative w-full h-full">
                    ...
                    <img
                      key={`nutraceutical-image-${currentImageIndex}`}
                      src={currentImageIndex === 0 ? nutraceutical.image : (nutraceutical.image2 || nutraceutical.image)}                     
                      alt={`${nutraceutical.productName || 'Product'} - Image ${currentImageIndex + 1}`}
                      className="w-full h-full object-contain transition-opacity duration-500"
                      onLoad={() => console.log('Nutraceutical image loaded:', currentImageIndex === 0 ? nutraceutical.image : nutraceutical.image2)}
                      onError={(e) => {
                        console.error('Nutraceutical image failed to load:');
                        const target = e.currentTarget;
                        
                        // If it's the second image that failed, fall back to first image
                        if (currentImageIndex === 1 && nutraceutical.image2 && nutraceutical.image2 !== nutraceutical.image) {
                          console.log('Falling back to first image');
                          target.src = nutraceutical.image;
                          return;
                        }
                    ...
                    {nutraceutical.image2 && nutraceutical.image2 !== nutraceutical.image && (
                      <>
                        {/* Left Arrow */}
                        <button
                          onClick={() => {
                            const newIndex = currentImageIndex === 0 ? 1 : 0;
                            console.log('Switching nutraceutical image to index:', newIndex);
                        ...
                        <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex space-x-2">
                          <button
                            onClick={() => {
                              console.log('Clicking nutraceutical indicator 0');
                          ...
                          <button
                            onClick={() => {
                              console.log('Clicking nutraceutical indicator 1');
                  ...
                  <div className="absolute top-3 right-3 z-20">
                    <div className={`flex items-center space-x-1.5 px-2.5 py-1.5 rounded-full text-xs font-medium ${
                      nutraceutical.inStock ? 'bg-green-500 text-white' : 'bg-red-500 text-white'
                    }`}>
                      <div className={`w-1.5 h-1.5 rounded-full ${nutraceutical.inStock ? 'bg-green-200' : 'bg-red-200'}`}></div>
                      <span>{nutraceutical.inStock ? 'In Stock' : 'Out of Stock'}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-7">
                  <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between mb-6">
                    <div>
                      <h1 className="text-3xl font-bold text-gray-800 mb-2">{nutraceutical.productName}</h1>
                      <p className="text-xl text-green-600 font-semibold mb-2">{nutraceutical.activeIngredient}</p>
                      <div className="flex items-center space-x-3 text-sm text-gray-600">
                        <div className="flex items-center space-x-1">
                          <Building2 className="w-3.5 h-3.5" />
                          <span>{nutraceutical.manufacturer}</span>
                        </div>
                        <div className="flex items-center space-x-1">
                          <Package className="w-3.5 h-3.5" />
                          <span>{nutraceutical.packSize}</span>
                        </div>
                      </div>
                    </div>
                    <div className="mt-3 lg:mt-0 text-right">
                      <div className="flex items-center space-x-2 mb-2">
                        <div className="flex items-center text-yellow-400">
                          {[...Array(5)].map((_, i) => (
                            <Star key={i} className={`w-5 h-5 ${i < Math.floor(nutraceutical.rating) ? 'fill-current' : ''}`} />
                          ))}
                        </div>
                        <span className="text-lg font-semibold text-gray-700">{nutraceutical.rating}</span>
                      </div>
                      <div className="text-3xl font-bold text-green-600">₨{nutraceutical.price.toLocaleString()}</div>
                      <div className="text-sm text-gray-500">{nutraceutical.dosageForm}</div>
                    </div>
                  </div>
                  ...
                  <div className="mb-6">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {nutraceutical.targetAnimals.map((animal) => (
                  ...
                  <div className="space-y-6">
                    {activeTab === 'overview' && (
                      <div className="space-y-5">
                        <div>
                          ...
                          <p className="text-gray-700 leading-relaxed text-sm">{nutraceutical.description}</p>
                        </div>
                        
                        <div>
                          <h3 className="text-lg font-semibold text-gray-800 mb-2.5">Indications</h3>
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
                            {nutraceutical.indications.map((indication, index) => (
                      ...
                      <div>
                        ...
                        <div className="bg-gray-50 rounded-lg p-5 border border-gray-200">
                          <div className="space-y-3">
                            {Object.entries(nutraceutical.composition).map(([ingredient, amount]) => (
                      ...
                      <div className="space-y-5">
                        <div>
                          ...
                          <div className="bg-green-100 rounded-lg p-3.5 border border-green-600">
                            <div className="flex justify-center space-x-2 mb-1.5">
                              ...
                              <span className="block font-medium text-green-800 text-sm text-center">Dosage Form: {nutraceutical.dosageForm}</span>
                            </div>
                          </div>
                        </div>
                        ...
                        <div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                            {Object.entries(nutraceutical.withdrawalPeriod).map(([product, period]) => (
                    ...
                    {activeTab === 'safety' && (
                      <div className="space-y-5">
                        ...
                        {nutraceutical.veterinaryPrescriptionRequired && (
            ...
            <div className="space-y-5">
              ...
              <div className="bg-white rounded-xl shadow-lg p-6">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center py-1.5 border-b border-gray-100">
                    ...
                    <span className="text-gray-600 text-sm">{nutraceutical.brandName}</span>
                  </div>
                  <div className="flex justify-between items-center py-1.5 border-b border-gray-100">
                    <span className="font-medium text-sm">Category</span>
                    <span className="text-gray-600 text-sm">{nutraceutical.category}</span>
                  </div>
                  <div className="flex justify-between items-center py-1.5 border-b border-gray-100">
                    <span className="font-medium text-sm">Pack Size</span>
                    <span className="text-gray-600 text-sm">{nutraceutical.packSize}</span>
                  </div>
                </div>
              </div>
              ...
              style={{paddingTop: '28px'}}>
                ...
                <div className="space-y-4">
                  {nutraceuticals
                    .filter(a => a.id !== nutraceutical.id && a.targetAnimals.some(animal => nutraceutical.targetAnimals.includes(animal)))
                    .slice(0, 3)
                    .map((related, index) => (
                      <div key={related.id}>
                        <Link href={`/nutraceuticals/${related.id}`}>
                ...
                <div className="flex items-start space-x-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-red-700 text-xs">
                      Always consult with a qualified veterinarian before administering nutraceutical supplements. 
                      Follow recommended dosages and usage guidelines strictly for optimal health benefits.
                    </p>
                  </div>
                </div>
              </div>
